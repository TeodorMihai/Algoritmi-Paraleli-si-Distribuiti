...
void findWay(int source, int dest, int* top, int n) {
...
int main(int argc, char * argv[]) {
	...
	*/
	...
	if(rank == 0) {
		...
		int nexthope = findWay(rank, dest, top);
		//find next hope
		for( ; top[nexthope] != rank;  nexthope = top[nexthope] );
		...
		MPI_Recv(emptys, sizeof(msg), MPI_CHAR, top[rank], 111, MPI_COMM_WORLD, &status);
		int dest;
		MPI_Recv(&dest, 1 , MPI_INT, top[rank], 222, MPI_COMM_WORLD, &status);
